...
def init_data():
    ...
    c.execute('''CREATE TABLE review
             ...
             no_of_positives INT  NOT NULL,
             no_of_negatives INT  NOT NULL,
             pos_perc        TEXT NOT NULL,
             neg_perc        TEXT NOT NULL);''')
    ...
    for i in range(len(foods)):
        c.execute("INSERT INTO review VALUES(:item_name,:no_of_customers,\
...
#Run only once to create a table review, then delete comment
#init_data()
...
def estimate(s):
    ...
    c.execute("SELECT *,oid FROM review")
    records = c.fetchall()
    ...
    for i in records:
        ...
        if rec[0] in selected_foods:
            ...
            c.execute("""UPDATE review SET Item_name=:item_name,No_of_customers\
            =:no_of_customers,no_of_positives=:no_of_positives,\
            no_of_positives=:no_of_negatives,pos_perc\
            =:pos_perc,neg_perc=:neg_perc  where oid=:Oid""",
                      {
                          'item_name': rec[0],
...
def login():
	...
	enter your restaurant's private code....",
...
root1 = Tk()
main = "Restaurant Review Analysis Sysstem/"
